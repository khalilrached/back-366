...
const storage = multer.diskStorage({
  destination(_req, _file, cb) {
    fs.mkdirSync('src/v1/services', { recursive: true });
...
sportsRoutes.post('/sport', uploadMany.fields([{ name: 'picture', maxCount: 1 }]), sportServices.getSportPredictions);
sportsRoutes.post(
  '/products',
  uploadMany.fields([{ name: 'picture', maxCount: 1 }]),
